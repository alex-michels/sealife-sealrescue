...
- Локализация — нативная Payload, контент-локали `ru` (исходная) + `en` — обе полноценные. Немецкий не является языком сайта: `de` остаётся только как **legal-only** route-локаль для четырёх обязательных страниц (Impressum / Datenschutz / Cookies / Terms), потому что оператор в Германии; контент под `/de` отдаёт 404.
