...
не превращаются автоматически в verified. Реализованный [M2-T13](proposal-review.md) заново читает target под блокировкой и сверяет `from`, чтобы не перетереть более свежую правку человека.
