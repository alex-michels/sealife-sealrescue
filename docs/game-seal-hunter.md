...
1. **Затемнённый фон оверлея** (стартовый + финальный экран, в **обоих** режимах — standalone и
   ...
   * **`center top`** (не `center`) — чтобы заголовок на самой обложке не обрезался сверху на широких
     ...
     вода, а не заголовок.
   * **Портрет/мобайл:** на портретных экранах `cover` режет широкую обложку **по бокам** (заголовок
     обрезается с краёв), позицией это не лечится. Поэтому медиа-запрос `(orientation: portrait)`
     **отключает картинку** — остаётся чистый тёмный фон, заголовок даёт `<h1>`. Нужен арт и на мобиле
     — добавить **портретную/квадратную** обложку отдельным `background` в этом медиа-запросе.
