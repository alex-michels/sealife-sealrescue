...
в `render/scenery.js` (`initBorder` строит сцену на ресайз; `drawDeepBackdrop` + `drawBorderDecor`
рисуют каждый кадр, анимация по времени, уважает `prefers-reduced-motion`). При contain-fit лишним
бывает **только одна ось**, поэтому экран показывает **либо** боковые стены (широкий), **либо**
поверхность+дно (высокий/портрет) — не одновременно:
...
* **Низ (портрет/4:3):** **песчаное дно** градиентом разных оттенков — рябь песка, галька, **коряги**,
  валуны, качающаяся травка (продолжение дна бухты ниже игрового поля).
* **Верх (портрет/4:3):** **поверхность воды и небо** — волнящаяся ватерлиния с пенным гребнем, воздух/
  туманное небо, плывущие облака, **иногда лодки** по поверхности, а если места сверху много (`ay0>90px`)
  — изредка **птицы**. Детализация (число облаков/птиц/лодок/элементов дна) **масштабируется** от
  доступной площади бордюра.

На 16:9 бордюра нет (арена = весь канвас). Тюлень упирается в кромку арены — граница видна и выглядит
задуманной.
...
бампать `CACHE` в `sw.js` (сейчас `seal-hunt-static-v7`; SW также не кэширует `/api/`).
