...
**Админка Payload (CR-17):** `i18n.supportedLanguages = { en, ru }`, `fallbackLanguage = en`.
Немецкий браузер и старая `payload-lng=de` cookie используют английский интерфейс;
явный сохранённый выбор RU/EN остаётся приоритетным. Контракт проверяется unit-тестом
на фактической конфигурации и штатной функции Payload `getRequestLanguage`.
Legal-only `/de` относится только к публичным документам и не добавляет язык админки.
...
**CR-16 (2026-09-09):** legal-only пути проверяет `src/proxy.ts` ДО входа в списочный роут.
Точный allowlist `legalSlugs` из `src/site/legalRoutes.ts` сохраняет четыре legal-страницы;
остальные `/de` пути переписываются на `[site]/[locale]/not-found`, где синхронный `notFound()`
срабатывает без loading-границы и без чтения CMS. URL посетителя, английская обвязка,
брендинг выбранного сайта и немецкие legal-ссылки сохраняются. Скелеты валидных RU/EN
списков остаются; отсутствие скелета у квизов больше не является защитой `/de`.
Unit проверяет allowlist против legal-навигации, точный матч и rewrite-заголовки;
`legal.e2e.spec.ts` проверяет настоящий HTTP 404 всех разделов и деталей обоих сайтов,
HTTP 200 четырёх legal-страниц и прежний 404 UI. CI запускает эти проверки на прод-сборке.
