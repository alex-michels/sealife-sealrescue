...
Хук `markTranslationsStale` (см. [agents.md](agents.md)) помечает перевод устаревшим при изменении
исходника; исходная и целевые локали берутся из `defaultLocale`/`targetLocales`, поэтому разворот
направления (CR-14) хук не трогает. Сейчас: исходная — `en`, целевая одна — `ru`.
