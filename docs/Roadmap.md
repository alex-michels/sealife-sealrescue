...
* **Фаза 1 — Уборка и честность**
  ...
  * [x] **SH-02** **Фиксированная честная арена** одного размера для всех (`960×540`/`540×960`, площадь = 518 400 u²) → одинаковые поле, число рыбы, плотность и расстояния проплыва независимо от экрана; масштабируется только рендер (contain-fit + центрирование). Баланс как device-independent инварианты: плотность добычи на площадь, время жизни рыбы, время пересечения поля. Лишнее место по краям — **диегетический бордюр** (стены водорослей+камни по бокам; песчаное дно с корягами/галькой/травой снизу; поверхность с волнами/лодками/облаками/птицами сверху), а не чёрные полосы. *[M]*
