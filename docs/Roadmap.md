...
* [~] **EU-11** **AI Act transparency (Art. 50, к 08.2026):** метки AI-assisted / AI-translated / AI-checked / Human reviewed / Source verified; статус проверки человеком; disclosure, если появится чат-бот; маркировка дипфейков. *[M]*
  ...
  `tests/int/provenance.int.spec.ts`. Осталось пользовательское отображение: `AiBadge` пока рендерит
  одну общую метку, `VerificationStamp` кормится моками — до подключения к реальным полям обещание Art.
  50 видно в БД, но не читателю.
...
>
> **Почему это трек, а не «потом».** Сид биологии пишется в БД со статусом `published`
> (`src/seed/lib.ts`) и после отмены M1-T06 становится few-shot-образцом, по которому агент будет
> писать статьи (M2-T06). Ошибку уровня семейства дешевле убрать сейчас, чем после сотни
...
* [ ] **BIO-01** Добавить главное, чего нет вообще: **тюлень на берегу — обычно норма, а одинокий
...
* [ ] **BIO-02** Добавить запрет **«никогда не сталкивайте и не относите тюленя в воду»** — самая
...
* [ ] **BIO-03** Добавить предупреждение об **укусах и зоонозах**: тюлень кусает быстро и сильно;
...
* [ ] **BIO-04** Заменить пустое «оцените состояние с расстояния» списком **наблюдаемых признаков**:
...
* [ ] **BIO-05** Разделить сценарий квеста «тюлень выглядит вялым»: сейчас правильным ответом
...
* [ ] **BIO-06** Дать **нижнюю границу дистанции**. Решение «не хардкодить одно универсальное число»
...
**Фактические ошибки в опубликованном контенте** (сид пишется как `published`):

* [ ] **BIO-08** `how-seals-sleep` приписывает **однополушарный сон настоящим тюленям**. Это
...
* [ ] **BIO-09** Статус балтийской нерпы **противоречит сам себе**: сид сеет `Pusa hispida botnica`
...
* [ ] **BIO-10** Четыре неверных кода IUCN в глоссарии: гренландский тюлень NT→**LC** (~7–8 млн
...
* [ ] **BIO-11** Карточка серого тюленя: этимология названа **латынью**, тогда как *Halichoerus
...
* [ ] **BIO-12** Терминология haul-out: в опубликованной новости стоит английское **rookery** и
...
* [ ] **BIO-16** Прекратить сеять непроверенную биологию как `published` и промаркировать
