...
> дистанция = очки) — эталонная Phaser-игра проекта. Обязательно: **lazy-load бандла** (динамический
> `import()` внутри статической игры, НЕ в общий Next-чанк — бюджет CWV/DESIGN-03), изоляция как у
> Canvas2D-игры (iframe/route на `sealife.info/[locale]/games/seal-run`, БЕЗ отдельного домена в v1 —
> см. `docs/game-seal-run.md` §2.5), переиспользование анонимного server-authoritative лидерборда
> (SH-05/06/07, расширение полей — SR-09). Полный дизайн (биология/баланс/архитектура) —
> [`docs/game-seal-run.md`](game-seal-run.md).

* **Фаза 1 — Дизайн и harness баланса**
  * [ ] **SR-01** Спека механик: 3 яруса препятствий (хищники −жизнь / сети-мусор−замедление /
    камни−отскок), единый стамина/кислород-метр (восстановление рыбой), сид-детерминированная
    генерация трассы (`courseSeed = hash(season)`, общая трасса на неделю), фиксированная по
    дистанции сессия (не бесконечная). *[M]* → DESIGN
  * [ ] **SR-02** `core/chunks.js`: формат чанка (difficultyScore/biome/lanes/obstacles/fish) +
    библиотека 15–25 авторских чанков (один biome `coastal` в v1) + `generateCourse(seed, biome)`
    сид-детерминированный сборщик. *[M]*
  * [ ] **SR-03** `core/sim.js`+`core/balance.js` (DOM-free): физика тюленя по Y (arrive-steering,
    как в Seal Hunter), стамина/жизни/баффы/дебаффы как в спеке SR-01, константы из таблицы
    балансировки первого прохода. *[M]*
  * [ ] **SR-04** `tools/fairness-sim.mjs` + `tools/compare-variants.mjs`: headless-бот (фикс.
    политика уворота/сбора) по матрице устройств (как у Seal Hunter) × N сидов трассы; отчёт по
    разбросу дистанции/жизней/улова — и по девайсам, и по сидам. Гейт перед тюнингом баланса. *[M]*
* **Фаза 2 — Core-механика + Phaser-интеграция**
  * [ ] **SR-05** Vendored Phaser 3 (`public/games/seal-run-v1/vendor/phaser.esm.js`, статический
    файл, БЕЗ нового бандлера в проекте) + сцены Boot→Preload→MainMenu→Play→GameOver; Play-сцена —
    тонкий рендер-слой над `core/sim.js`/`core/chunks.js` (object pooling для рыбы/препятствий,
    Arcade Physics). Динамический `import()` вендор-бандла ТОЛЬКО по нажатию «Старт». *[L]* → PERF
  * [ ] **SR-06** Арт: спрайт-атлас тюленя (профиль сбоку — плавание; задел под будущую
    «галумпинг»-анимацию, не для v1) + хищники (орка/акула/белый медведь + элитный леопардовый
    тюлень) + мусор (сеть/пластик) + камни/рыба; тинт из **semantic-токенов** (Foggy Coastal
    Utility), параллакс-фон (2–3 слоя). *[L]* → DESIGN
  * [ ] **SR-07** HUD (жизни/стамина-бар/дистанция) в safe-area, HTML-инструкции и HTML-результат
    ВНЕ canvas (доступность), WCAG 2.2 AA (focus/target≥24px/`prefers-reduced-motion`), яруса
    препятствий различимы НЕ только цветом (силуэт). *[M]* → DESIGN/EU
* **Фаза 3 — Лидерборд/бэкенд**
  ...
  * [ ] **SR-09** Расширить `game-scores`: опциональные поля `distance`/`livesRemaining`/
    ...
    существующих строк); `score` остаётся required сорт-ключом (производная формула от
    дистанции+улова+жизней), код ранжирования в `leaderboard.ts` не меняется. `generate:types` +
    обновить `data-model.md`. *[M]*
  * [ ] **SR-10** Anti-cheat формула под раннер: заменить фикс-окно длительности (было 50–70с под
    60с-раунд Seal Hunter) на «правдоподобная длительность от заявленной дистанции»
    (`MAX_SPEED_UNITS_PER_MS`) + серверная регенерация трассы по `courseSeed` и сверка
    `fishCollected` с бюджетом рыбы на трассе до заявленной дистанции (сильнее статистических
    капов Seal Hunter — возможно ТОЛЬКО благодаря сид-детерминированной трассе). *[M]* → SEC
  * [ ] **SR-11** `core/alias.js`+`i18n.js`+`sw.js` для seal-run-v1 — копии структуры Seal Hunter
    (свой словарь строк, свой `CACHE`-конст, свой `localStorage`-ключ `seal_run_seed`), синхронно с
    серверным `makeParts`/word-lists (тот же общий сервер-эндпоинт, только новый `game`-slug). *[S]*
* **Фаза 4 — Доступность/compliance/публикация**
  * [ ] **SR-12** Встраивание на `sealife.info` (и, если уместно тону, `sealrescue.info`) через
    существующий `/[site]/[locale]/games/[slug]` роут — БЕЗ нового домена/поддомена в v1 (см.
    `docs/game-seal-run.md` §2.5: `sealife.info` ещё не в проде + не повторять SH-10-разрыв на
    третьем происхождении). `sealrun.sealife.info` — возможный будущий vanity-алиас, ревью ПОСЛЕ
    прод-запуска `sealife.info` и ПОСЛЕ SH-10. *[S]*
  ...
  * [ ] **SR-14** E2E (`tests/e2e/game-seal-run.e2e.spec.ts` по конвенции `game-standalone.e2e.spec.ts`,
    без standalone-ветки) + юнит-тест детерминизма `generateCourse(seed)` (тот же сид → байт-в-байт
    одинаковая трасса — свойство, на которое опирается SR-10). *[M]* → QA
...
* [ ] **M4-T05** 🎮 idle «собери лежбище» (Phaser-игра — «Seal Run» уже расписана отдельно, см. **SR-01..SR-14**). *[L]*
