...
* **Фаза 5 — Дальнейшая честность и контент (backlog)**
  ...
  * [ ] **SH-13** ⚡ **ОЧЕНЬ РЕДКАЯ добыча «lightning sea star»** (молниевая морская звезда): при
    поимке — бафф **×2 скорости тюленя на 2 с** (+ обычное 1 очко как у прочей добычи — счёт-модель
    не меняется). Спавн: **очень редкий**, случайное время и случайная позиция (отдельный редкий
    roll ВНЕ edge-shuffle-bag обычной добычи; ориентир — вероятность появления ≪1 за раунд,
    максимум 1 за раунд; тюнить харнессом). Вид: силуэт звезды с зигзаг-лучами, свечение +
    молнии-эффект (процедурный вектор, как прочая добыча; вспышки глушатся
    `prefers-reduced-motion`; различимость НЕ только цветом — силуэт). Техника: временный множитель
    скорости в `core/sim.js` (сейчас модификаторов скорости нет — новый механизм), спавн через
    seeded-RNG (иначе рассыпется детерминизм golden/fairness-тестов QA-30/31 — обновить осознанно);
    проверить, что бафф-бёрст не упирается в серверный кап правдоподобия
    (`CATCH_PER_SEC_CAP`, `implausible_score`). *[M]* → DESIGN
