...
Что означает каждый тип, попадает ли он в ленту главной и как подписывается на двух языках —
`src/content/contentTypes.ts` (CR-06). Таблица там исчерпывающая по union `Content['type']`:
новый тип не соберётся, пока для него не принято решение «редакционный или служебный».
`page` в ленту **не** попадает — служебные страницы (about, контакты, редполитика) живут по прямой
ссылке и в футере.
