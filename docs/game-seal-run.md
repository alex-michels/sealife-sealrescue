...
§2.9), реализация — по задачам SR-01..SR-15.
...
— берётся из токена. Без изменений: `durationMs`, `board`, `seed`, `token`.
...
### 2.8 Тестирование
`tools/fairness-sim.mjs` — бот с фикс. политикой **непрерывного Y-контроля** (плыть к безопасной
Y-полосе; заходить за рыбой, если путь к ней не пересекает угрозу) по матрице устройств Seal Hunter ×
N `courseSeed` — разброс дистанции/жизней/улова по ДВУМ осям (устройство — как у Seal Hunter, сид —
новая для Seal Run). Гейт перед тюнингом баланса, как и у Seal Hunter. `tools/compare-variants.mjs` —
тот же common-random-numbers A/B для тюнингов (напр. множитель замедления от мусора) до фиксации
значения. E2E (`tests/e2e/game-seal-run.e2e.spec.ts`, по конвенции `game-standalone.e2e.spec.ts`, БЕЗ
