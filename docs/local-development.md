...
- **Integration** — Vitest project `int` (`tests/int/*.int.spec.ts`, jsdom + setup; файлы бегут
  ...
  наперегонки).
  ⚠️ **Локальная дев-БД удалённая (Neon, eu-central-1), и это иногда видно в e2e.** Хуки, которые
  поднимают Payload (`seo`, `preview`, `admin` через `seedUser`), ждут коннекта к удалённой БД;
  на «холодной» Neon это иногда превышает таймаут хука, и Playwright показывает падение как флаку
  в первом тесте файла (например «главная /ru: canonical…»), хотя сам тест ни при чём — в логе
  видно `"beforeAll" hook timeout … exceeded` на `getPayload()`. Таймауты таких хуков подняты до
  120 с, но это страховка, а не устранение причины. **В CI не воспроизводится структурно:** там
  эфемерный локальный Postgres в service-контейнере и `workers: 1`. Если мешает локально —
  поднять Postgres рядом и указать его в `DATABASE_URI`.
