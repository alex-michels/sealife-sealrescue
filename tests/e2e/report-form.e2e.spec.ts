 ...
 *  - за гарантированным ответом — ссылка на контакт оператора в legal-notice текущей локали
 *    (на legal-only /de это немецкий Impressum), это contact point оператора, не сбор данных;
 *  - error-состояние: без обязательного сообщения форма не уходит;
 *  - success-состояние после отправки (role=status).
 *
 * Форма — контентный роут, поэтому живёт только на ru/en: /de/report отдаёт 404
 * (проверяется в legal.e2e.spec.ts), немецкоязычный посетитель приходит на /en/report.
 *
