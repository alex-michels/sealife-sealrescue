...
* **Любая фича без теста — баг.** Тесты — часть Definition of Done наравне с доками: новая фича, бизнес-логика или изменение поведения приходит в PR вместе с тестом соответствующего уровня (unit / integration / e2e; карта уровней и конвенции — `docs/local-development.md` § Тесты, план покрытия — `docs/Roadmap.md` § QA). Изменение, валящее существующий тест, чинит код либо осознанно обновляет тест **в том же PR**; неактуальный (устаревший, скипнутый, флаки) тест — такой же баг, как его отсутствие.
* **Перед КАЖДЫМ коммитом/пушем — полный локальный прогон:** `npm run lint` + `npm run typecheck` + `npm run test:coverage` (unit+int); менял фронтенд, схему Payload или конфиги сборки — ещё и `npm run build`; затронул e2e-поведение — прогнать соответствующие e2e-спеки. Красное = не коммитим. Чек-лист — `docs/local-development.md` § «Перед коммитом».
* Маленькие PR, осмысленные коммиты (conventional commits).
* После изменения схемы Payload: `npm run generate:types`; миграцию БД ревьюить до применения. **NB:** схема пока push-based — локальный `npm run build` проходит против ТВОЕЙ БД (схема уже запушена), но чужие БД новых таблиц не имеют: build-/alpha-БД деплоя получает push автоматически (шаг «Sync DB schema» в `deploy.yml`), CI-БД — в jobs `test`/`e2e`; новые окружения — см. `docs/DEPLOYMENT.md` §9.
